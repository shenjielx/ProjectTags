...
namespace ProjectTags.Unitil
{
    public class ToolPager
    {
        ...
        public static string UidPrefix = string.Empty;

        public static string GetJumper()
        {
        ...
        /// <returns></returns>
        public static string GetJumper(int pageIndex, int pageSize, int recordCount, string className = "pagination")
        {
            PageIndex = pageIndex;
            PageSize = pageSize;
            ...
            printer.AppendFormat("<div class=\"{0} fl\"><ul>", className);
            ...
            for (int i = iFirst; i <= iLast; i++)
            {
                if (i == PageIndex)
                {
                    printer.AppendFormat("<li class=\"active\" title=\"第{0}页\"><a href=\"javascript:;\">{0}</a></li>", i);
                }
                else
                {
        ...
        /// <returns></returns>
        public static string GetJumperForAjax(int pageIndex, int pageSize, int recordCount, string ajaxProviderFormat, string className = "pagination pagination-sm", string code = "")
        {
            PageIndex = pageIndex;
            PageSize = pageSize;
            RecordCount = recordCount;
            ...
            if (PageIndex < 7) { iLast = 10; }
            if (iLast > PageCount) { iLast = PageCount; }
            printer.AppendFormat("<div><ul class=\"{0}\">", className);

            bool isFirst = PageIndex <= 1;
            bool isLast = PageIndex >= PageCount;
            printer.AppendFormat("<li class='{0}' {1}><a>首页</a></li>", isFirst ? "disabled " : "", isFirst ? "" : string.Format("title=\"转到首页\" href=\"javascript:void(0);\" onclick=\"{0}\"", string.Format(ajaxProviderFormat, 1)));
            printer.AppendFormat("<li class='{0}' {1}><a>« </a></li>", isFirst ? "disabled" : "", isFirst ? "" : string.Format("title=\"转到上一页\" href=\"javascript:void(0);\" onclick=\"{0}\"", string.Format(ajaxProviderFormat, PageIndex - 1)));
            ...
            for (int i = iFirst; i <= iLast; i++)
            {
                ...
                {
                    printer.AppendFormat("<li title=\"转到第{0}页\" ><a href=\"javascript:void(0);\" onclick=\"{1}\">{0}</a></li>", i, string.Format(ajaxProviderFormat, i));
                }
            }
            printer.AppendFormat("<li class='{0}' {1}><a>»</a></li>", isLast ? "disabled " : "", isLast ? "" : string.Format("title=\"转到下一页\" href=\"javascript:void(0);\" onclick=\"{0}\"", string.Format(ajaxProviderFormat, PageIndex + 1)));
            printer.AppendFormat("<li class='{0}' {1}><a>尾页</a></li>", isLast ? "disabled " : "", isLast ? "" : string.Format("title=\"转到尾页\" href=\"javascript:void(0);\" onclick=\"{0}\"", string.Format(ajaxProviderFormat, PageCount)));
            //<span class=\"goto\">到第<input type=\"text\" value=\"{1}\" size=\"3\" id=\"txtGoPage" + code + "\" />页</span><a href=\"javascript:void(0);\" title=\"跳转页面\" onclick=\"{2}\">GO</a>", RecordCount, pageIndex, string.Format(ajaxProviderFormat, "0"));
            printer.Append("</ul>");
        ...
        /// <returns></returns>
        public static string GetPagerHtml(int pageIndex, int pageSize, int recordCount, int houseID)
        {
        ...
        public static string GetJumper(int pgNumber, int pgSize, int rdCount)
        {
            PageIndex = pgNumber;
            PageSize = pgSize;
            RecordCount = rdCount;
        ...
        public static string GetJumper(int pgSize, int rdCount)
        {
            GetPageNumber();
            PageSize = pgSize;
            RecordCount = rdCount;
        ...
        private static void GetPageNumber()
        {
            int pn = 0;
        ...
        private static void ComputePageNumber()
        {
